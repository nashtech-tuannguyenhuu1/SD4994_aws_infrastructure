...
output "ssh_connection_string_for_ec2" {
  value = format("%s%s", "ssh -i C:/Users/tuannguyenh1/.ssh/jenkins_msa ubuntu@", aws_instance.jenkins_ec2_instance_ip.public_ip)
}
...
output "msg" {
  value = "Please wait 5-10 mintues until ${var.tag_name} is up and running."
  depends_on = [
    aws_instance.jenkins_ec2_instance_ip
  ]
}